...
params:
   input: 
      ...
      input: file
      value: "https://raw.githubusercontent.com/immcantation/enchantr/refs/heads/master/data-raw/ExampleDb.tsv.gz"
   outname:
      label: "`outname`"
      input: text
      value: "chimera"
...
if (!is.null(params$outname)) {
    output_fn <- paste0(params$outname,"_chimera-pass.tsv.gz")
} else {
    output_fn <- sub("\\.tsv(\\.gz)?$", "_chimera-pass.tsv.gz", basename(params$input))
}

if (!is.null(params$log)) {
    log_fn <- paste0(params$log,".txt")
} else {
    log_fn <- sub("_chimera-pass.tsv.gz$", "_command_log.txt", basename(output_fn))
}
